        ...
        for key, values in self.bn.items():
            ...
            if key == "random_variables":
                ...
                for value in values.split(";"):
                    if "(" in value and ")" in value:
                        ...
                        if len(parts) > 1:
                            ...
                            var_set.append(parts[0].strip())
                    else:
                        var_set.append(value.strip())
                self.bn[key] = var_set
                ...
                for value in values.split(";"):
                    ...
                    pair = value.split("=")
                    if len(pair) != 2:
                        print(f"⚠️ Skipping malformed CPT entry: {value}")
                        continue

                    try:
                        cpt[pair[0].strip()] = float(pair[1].strip())
                        total += float(pair[1].strip())
                    except ValueError:
                        print(f"⚠️ Could not convert probability to float in line: {value}")
                        continue

                print(f"key={key}, cpt={cpt}, sum={total}")
                self.bn[key] = cpt
